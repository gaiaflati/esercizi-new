...
- Definire i seguenti metodi:
    ...
    deve essere riaggiunto alla lista dei film disponibili in negozio (film_list). Successivamente, il metodo deve calcolare la penale che il cliente deve pagare 
    utilizzando l'opposito metodo. Stampare la penale che il cliente deve pagare: "Cliente: {clientID}! La penale da pagare per il film {titolo_film} e' di {tot} euro!".
...
class Noleggio:
    ...
    def __init__(self) -> None:
        ...
        self.rented_film: dict = {list[Film]}
    ...
    def rentAMovie(self, film: Film, clientID): 
        if self.isAvaible(film) == True:
            ...
            return f"Il cliente {clientID} ha noleggiato {film.title}!"
        else:
            return f"Non è possibile noleggiare il film {film.title}"
...
#utilizzando l'opposito metodo. Stampare la penale che il cliente deve pagare: "Cliente: {clientID}! La penale da pagare per il film {titolo_film} e' di {tot} euro!".
    ...
    def giveBack(self, film: Film, clientID, days: int):
        for i,j in self.rented_film.items():
            if film in j:
                self.rented_film.
